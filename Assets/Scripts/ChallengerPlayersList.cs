...
public class ChallengerPlayersList : MonoBehaviour
{
    ...
    public void ChangePlayerTurn()
    {
        ...
        StartCoroutine(FadeOutHealthStatusBox());
    ...
    public void ChangePlayerHealth(int playerIndex)
    {
        bool health = playerList[playerIndex].healthStatus;
        //Se estiver saudável, é infectado; caso contrário, é curado.
        if (health == true)
        {
            health = false;
            healthStatusBox.GetComponentInChildren<Text>().text = playerList[playerIndex].name + ", '\nvocê agora tem COVID-19!";
        }
        else
        {
            health = true;
            healthStatusBox.GetComponentInChildren<Text>().text = playerList[playerIndex].name + ", '\nvocê agora está SAUDÁVEL!";
        }
